...
export function getUserStats(userId: number): UserAchievementStats {
...
export function recordSoloQuizComplete(
  userId: number,
  correct: number,
  total: number,
  avgSpeedMs: number,
...
export function recordGroupQuizParticipation(
  userId: number,
  isWinner: boolean,
  correct: number,
  total: number,
...
export function recordFirstQ1Correct(userId: number): string[] {
  const s = getUserStats(userId);
...
export function recordLeaderboardPosition(
  userId: number,
